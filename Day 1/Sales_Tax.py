...
class Window(customtkinter.CTk):
    ...
    def calculate(self):
        ...
        self.receipt_cost = round((self.total_labour_cost+self.total_paint_cost), 2)
    ...
    def disp(self):
        ...
        self.frame1 = customtkinter.CTkFrame(
            master=self, corner_radius=20, width=300, height=200)
        self.frame1.pack(fill=tk.Y, expand=1, pady=30, padx=30)
        # creating receipt labels
        self.label1 = customtkinter.CTkLabel(
            master=self.frame1, text=self.output[0])
        self.label1.pack(pady=20, padx=50, side=tk.TOP)
        self.label1 = customtkinter.CTkLabel(
            master=self.frame1, text=self.output[1])
        self.label1.pack(pady=20, padx=50, side=tk.TOP)
        self.label1 = customtkinter.CTkLabel(
            master=self.frame1, text=self.output[2])
        self.label1.pack(pady=20, padx=50, side=tk.TOP)
        self.label1 = customtkinter.CTkLabel(
            master=self.frame1, text=self.output[3])
        self.label1.pack(pady=20, padx=50, side=tk.TOP)
        self.label1 = customtkinter.CTkLabel(
            master=self.frame1, text=self.output[4])
        self.label1.pack(pady=20, padx=50, side=tk.TOP)
        # creating exit button
        self.exit = customtkinter.CTkButton(
            master=self, text='EXIT', command=self.destroy)
